#!/bin/sh
# Run one of the steward plugin's scripts with a Python 3 that actually works.
...
stamp="$HOME/.claude/steward/no-python.$(date +%Y%m%d)"
if [ ! -f "$stamp" ]; then
  mkdir -p "$HOME/.claude/steward" 2>/dev/null && : > "$stamp" 2>/dev/null
  echo "STEWARD PLUGIN: no working Python 3 was found on this machine, so your steward cannot run."
  echo "Tell the person once, plainly: install Python 3 (python.org), then restart Claude Code."
fi
exit 0
